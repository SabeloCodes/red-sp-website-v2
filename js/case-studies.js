document.addEventListener("DOMContentLoaded", () => {
          ...
          tabs.forEach(tab => {
              ...
              if (header && content) {
                  ...
                  header.addEventListener('click', () => {
                      ...
                      // Close all tabs *before* deciding to open the clicked one
                      tabs.forEach(t => {
                      ...
                      if (!isActive) {
                          ...
                          // --- Initialize Mini Carousels INSIDE the opened tab ---
                  ...
                  // For any tabs that should be open by default on page load:
                  // Check if a tab is already active (e.g., from initial HTML or previous state)
  ...
  function initializeSingleMiniCarousel(carousel) {
      ...
      // Ensure only direct children are considered, or adjust selector if nested
      ...
      let currentIndex = 0;
      let modalIndex = 0; // Keep modal index local to this carousel instance if needed, or global if only one modal exists.

      // Get modal elements from the DOM (assuming modal-gallery.js injects it or it's directly in HTML)
      ...
      // Create dot indicators (only if dotsContainer exists and no dots are already there)
      const dots = [];
      if (dotsContainer && dotsContainer.children.length === 0) {
          items.forEach((_, i) => {
           ...
           // If dots already exist (e.g., re-opening tab), get them
      ...
      function openModal(index, sourceCarousel) {
          ...
          modalIndex = index; // Set modal index for global modal
      ...
      function showModalImage(index) {
          if (!modalImg || !modalCaption) return; // Exit if modal parts not found
      ...
      // Attach modal event listeners once globally if modal-gallery.js isn't handling it.
      // Assuming modal-gallery.js handles these, so we're just making sure it exists.
      // If modal-gallery.js truly is just for the modal itself and not the mini-carousel,
      // you'd need these listeners here or in modal-gallery.js.
      ...
      function updateCarousel() {
          ...
          items.forEach((item, i) => {
              item.classList.remove('active', 'prev-active', 'next-active'); // Clear all roles
              item.onclick = null; // Clear click listener
          });

          // Set classes based on role
          items[prevIndex].classList.add('prev-active');
          items[currentIndex].classList.add('active');
          items[currentIndex].onclick = (e) => { // Attach click listener only to active
              e.stopPropagation();
          ...
          // Re-check image loading status *after* display properties are potentially set by CSS
          items.forEach((item, i) => {
              const img = item.querySelector('img');
              if (img) {
                  if (!img.complete) {
                      console.warn(`Image at index ${i} not yet loaded: ${img.src}`);
                      img.onload = () => { console.log(`Image at index ${i} loaded successfully: ${img.src}`); updateCarousel(); }; // Re-update on load
      ...
      // Initial update
      updateCarousel();
      carousel.dataset.initialized = "true"; // Mark as initialized
  }
  ...
  // Removed `initializeMiniCarousel()` from here. It will be called by `setupTabbedSections`.
  ...
  injectContent('components/footer.html', 'page-footer', () => {
      ...
      setupTabbedSections(); // Call setupTabbedSections AFTER footer and its potential content is injected
  });

  setupTestimonialCarousel();

  // If you want the first tab to be open by default on page load and its carousel initialized:
  // (You might already have a CSS rule for `tab-section.active` in case-studies.css)
...
// Make initializeSingleMiniCarousel globally accessible if needed by other modules,
// but it's better to manage within this file's scope.
// window.initializeSingleMiniCarousel = initializeSingleMiniCarousel;